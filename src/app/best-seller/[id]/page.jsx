...
import { checkLanguage, FORMAT_NUMBER, formatCurrency } from "../../../../utils/helper";
import "../page.scss";
export default function page() {
  ...
  const { data: companyDetailData, refetch, isPending } = useQuery({
  ...
  const { data: classificationList, refetch: classificationData } = useQuery({
  ...
  const { data: classificationProductList, refetch: classificationRefetch, isPending: isPendingClassification } =
    useQuery({
  ...
  return (
    <>
      ...
      <Breadcrums firstLink={"Home"} secondLink={"Best Seller Detail"} />
        ...
        <Container>
          ...
          <div className="dashboard_content mt-5">
            <div className="inner_card">
              <Row>
                ...
                <Col lg={6}>
                  <small className="text-muted">Coupon Service</small>
                  <p>
                    {companyDetailData?.couponService == true ? "Yes" : "No"}{" "}
                  </p>
                  <hr />
                </Col>
                <Col lg={6}>
                  <small className="text-muted">Pickup Service</small>
                  <p>
                    {" "}
                ...
                <Col lg={6}>
                  <small className="text-muted">Self Delivery</small>
                  <p>
                    {" "}
                ...
                <Col lg={6}>
                  <small className="text-muted">Delivery Cost ({formatCurrency("", selectedCountry)})</small>
          ...
          <TabContent activeKey={activeTab}>
            <Row>
              ...
              {isPendingClassification ? Array.from({ length: 4 }, (_, index) => (
                <Col lg={3} className="mb-4 pb-5" key={index}>
                  <ShimmerPostItem
                    variant="primary" imageHeight={357} /></Col>
              )) : classificationProductList?.length !== 0 ? (
                classificationProductList?.map((data) => {
                  return (
                      ...
                      <div className="product-box-3  product-new">
                        ...
                        <div className="product-detail text-center mt-4">
                          ...
                          <p className="notranslate">
                            {formatCurrency(data?.size?.at(0)?.price
                              ? data?.size?.at(0)?.price
                              : data?.price, selectedCountry)}


                            {data?.size?.at(0)?.discount ? (
                              <del>{formatCurrency(data?.size?.at(0)?.mrp, selectedCountry)}</del>
                            ) : data?.discount ? (
                              <del>
                                {" "}

                                {formatCurrency(data?.size?.at(0)?.mrp
                                  ? data?.size?.at(0)?.mrp
                                  : data?.mrpPrice, selectedCountry)}
                              </del>
                            ) : (
                              ""
                            )}
                            {data?.discount !== null ? (
                              <span>{FORMAT_NUMBER(data?.discount, true)}% off</span>
                            ) : (
                              ""
                            )}
              ...
              {!isPendingClassification ? Math.ceil(meta?.totalCount / 10) > 1 && (
                <Pagination
                  pageCount={"YES"}
