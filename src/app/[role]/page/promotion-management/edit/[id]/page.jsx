...
const Edit = () => {
  ...
  const {
    ...
    validationSchema: yup.object().shape({
      ...
      //   .matches(/^(?!0+$)[0-9]+$/, "Value cannot be 0, 00, etc.")
      //   .test("is-valid-discount", "Value must be a valid number", (value) => {
      //     return value !== "0" && value !== "00" && value !== null;
      //   }),
      type: yup.string().required("Type is required"),
      startDate: yup.string().label("Select startDate").required(),
      endDate: yup.string().label("Select endDate").required(),
    ...
    onSubmit: async (values) => {
      ...
      if (values?.type == 2) {
        (payload.categoryId = values?.categoryId?.value ?? ""),
          (payload.subcategoryId = values?.subcategoryId?.value ?? "");
        payload.excludedCompany = values?.excludedCompany ? values?.excludedCompany?.map((data) => data?.value) : ""
      }
      if (values?.type == 3 && values?.company) {
        payload.company = values?.company?.map((data) => data?.value);
      }
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      if (resp?.data?.data?.type == 2) {
        ...
        excludedCompany =
          resp?.data?.data?.excludedCompany?.map((data) => ({
            value: data?._id ?? "",
            label: data?.company ?? "",
          }))
      ...
      if (resp?.data?.data?.type === 3) {
        company =
          resp.data.data.companyDetails?.map((data) => ({
            label: data.company ?? "",
            value: data._id ?? "",
          })) ?? [];
      }

      // if (resp?.data?.data?.actionType == 2) {
      cashBack = resp?.data?.data?.cashBackType;
      rotationCashBack = resp?.data?.data?.rotationCashBack;
      cashbackvalidity = resp?.data?.data?.cashbackvalidity;
      // }
      ...
      setValues({
        ...
        rotationCashBack,
        cashbackvalidity,
        supplierShare: resp?.data?.data?.supplierShare,
        company,
      ...
      <div className={isSlider ? "bodymain" : "bodymain collapsebody"}>
        <div className="breadcrum-top mb-4">
          <ul className="d-flex align-items-center gap-2">
            ...
            <li>/</li>
            <li className="text-capitalize">Edit Promotion</li>
          </ul>
        </div>
        <div className="row">
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
                <h5 className="mb-md-0">Edit Promotion</h5>
                <Link
                  href={getLinkHref(
                    detail?.roleId,
                    ...
                    {values?.actionType == "2" && (
                      <>
                        <Col lg={6} className="mb-3">
                          ...
                          {/* Add margin-bottom to create space */}
                          <div>
                            <Form.Label>Cash Back Type</Form.Label>
                            <Form.Select
                              aria-label="Select Cash Back Type"
                              ...
                              className="form-control"
                            >
                              <option value="">Cash Back Type</option>
                      ...
                      <Form.Group className="mb-4">
                        <Form.Label>
                          {(values?.cashBack == 1 && values?.actionType == 2) ||
                            values?.actionType == 1
                            ? `Discount(%)`
                            : `Amount (${formatCurrency("", values?.country)})`}
                          <span className="text-danger">*</span>
                        </Form.Label>
                        <Form.Control
                          className="form-control"
