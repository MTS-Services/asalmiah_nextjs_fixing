...
import {
  EDIT_DELIVERY_COMPANY_API,
  GET_DELIVERY_COMPANY_DETAIL,
...
const Edit = () => {
  ...
  const { mutate, error, isPending } = useMutation({
    mutationFn: (payload) => EDIT_DELIVERY_COMPANY_API(id, payload),
    onSuccess: (resp) => {
  ...
  const {
    ...
    validationSchema: yup.object().shape({
      company: yup.string().required().label("Company").trim(),
  ...
  useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const resp = await GET_DELIVERY_COMPANY_DETAIL(id);
      setValues({
        ...values,
        ...
        companyCode: resp?.data?.data?.companyCode,
        mobile: String(resp?.data?.data?.mobile),
        ...
        contactPersonName: resp?.data?.data?.contactPersonName,
        contactPersonMobile: String(resp?.data?.data?.contactPersonMobile),
        active: resp?.data?.data?.active,
        costDeliveryOffrat: resp?.data?.data?.costDeliveryOffrat,
        costDeliveryCustomer: resp?.data?.data?.costDeliveryCustomer,
        default: resp?.data?.data?.default,
        // ARABIC
        arabicCompany: resp?.data?.data?.arabicCompany,
        arabicDescription: resp?.data?.data?.arabicDescription,
        arabicRegistration: resp?.data?.data?.arabicRegistration,
        arabicContactPersonName: resp?.data?.data?.arabicContactPersonName,
      });
      ...
      <div className={isSlider ? "bodymain" : "bodymain collapsebody"}>
        <div className="breadcrum-top mb-4">
          <ul className="d-flex align-items-center gap-2">
            ...
            <li>/</li>
            <li className="text-capitalize">Edit Delivery Company</li>
          </ul>
        </div>
        <div className="row">
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
                <h5 className="mb-md-0">Edit Delivery Company </h5>
                <Link

                  href={getLinkHref(detail?.roleId, `/page/delivery-company-management`)} className="btn_theme"
                >
                  Back
                </Link>
              </div>
              <div className="card-body">
                <form onSubmit={handleSubmit}>
                  <Row>
                    ...
                    <Col lg={6}>
                      <div className="active-radio d-flex algn-items-center">
