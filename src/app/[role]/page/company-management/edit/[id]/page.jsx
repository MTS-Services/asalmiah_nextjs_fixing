...
const Edit = () => {
  ...
  const { mutate, error, isPending } = useMutation({
    mutationFn: (payload) => EDIT_COMPANY_API(id, payload),
    onSuccess: (resp) => {
  ...
  useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const resp = await GET_COMPANY_DETAIL(id);

      setValues({
        ...values,
        commissionType: resp?.data?.data?.commissionType,
        company: resp?.data?.data?.company,
        country: resp?.data?.data?.country,

        company_logo: resp?.data?.data?.logo,
        coverImg: resp?.data?.data?.coverImg,
        description: resp?.data?.data?.description,
        perCommission: resp?.data?.data?.perCommission
          ? resp?.data?.data?.perCommission
          : "",
        couponService: resp?.data?.data?.couponService == true ? 1 : 0,
        deliveryEligible: resp?.data?.data?.deliveryEligible == true ? 1 : 0,
        pickupService: resp?.data?.data?.pickupService == true ? 1 : 0,
        deliveryServiceEligible:
          resp?.data?.data?.deliveryService == true ? 1 : 0,
        order: resp?.data?.data?.order,
        // costDelivery: resp?.data?.data?.deliveryCompany?.company
        //   ? resp?.data?.data?.deliveryCompany?.costDeliveryCustomer
        //   : resp?.data?.data?.costDelivery
        //   ? resp?.data?.data?.costDelivery
        //   : "",

        costDelivery: resp?.data?.data?.costDelivery
          ? resp?.data?.data?.costDelivery
          : "",

        // Conditionally set deliveryCompanyId
        deliveryCompanyId: resp?.data?.data?.deliveryCompany
          ? {
            label: resp.data.data.deliveryCompany.company,
            value: resp.data.data.deliveryCompany._id,
          }
          : "",

        // Conditionally set categoryId
        categoryId: resp?.data?.data?.categoryDetails
          ? {
        ...
        // Conditionally set subcategoryId
        subcategoryId: resp?.data?.data?.subCategoryDetails
          ? {
        ...
        paymentPeriod: resp?.data?.data?.paymentPeriod,
        mobile: resp?.data?.data?.countryCode + resp?.data?.data?.mobile,
        email: resp?.data?.data?.email,

        // ARABIC
        arabicCompany: resp?.data?.data?.arabicCompany,
        arabicDescription: resp?.data?.data?.arabicDescription,
        actualCompanyName: resp?.data?.data?.actualCompanyName,
        arabicActualCompanyName: resp?.data?.data?.arabicActualCompanyName,
        deliveryCompanyChecked: resp?.data?.data?.deliveryCompanyChecked,
      });
      ...
      <div className={isSlider ? "bodymain" : "bodymain collapsebody"}>
        <div className="breadcrum-top mb-4">
          <ul className="d-flex align-items-center gap-2">
            ...
            <li>/</li>
            <li className="text-capitalize">edit Company</li>
          </ul>
        </div>
        <div className="row">
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
                <h5 className="mb-md-0">Edit Company</h5>
                <Link
                  href={getLinkHref(detail?.roleId, "/page/company-management")}
                  className="btn_theme"
                >
