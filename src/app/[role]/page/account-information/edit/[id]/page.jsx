...
import { countries, getLinkHref, restrictAlpha } from "../../../../../../../utils/helper";
...
const Edit = () => {
  ...
  const { mutate, error, isPending } = useMutation({
    mutationFn: (payload) => UPDATE_ACCOUNT(id, payload),
    onSuccess: (resp) => {
  ...
  useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const resp = await ACCOUNT_INFORMATION_DETAIL(id);
      setValues({
        ...values,
        company: {
        ...
        accountType: resp?.data?.data?.accountType,
        paymentMethod: resp?.data?.data?.paymentMethod,
        flexiblePrice: resp?.data?.data?.flexiblePrice,
        accountantName: resp?.data?.data?.accountantName,
        accountantTelephone: resp?.data?.data?.accountantTelephone,
        chequeCompany: resp?.data?.data?.chequeCompany,
        linkTelephoneNumber: resp?.data?.data?.linkTelephoneNumber,
        bankName: resp?.data?.data?.bankName,
        accountNumber: resp?.data?.data?.accountNumber,
        bankCode: resp?.data?.data?.bankCode,
        branchName: resp?.data?.data?.branchName,
        swiftCode: resp?.data?.data?.swiftCode,
        IBAN: resp?.data?.data?.IBAN,
        paymentPeriod: resp?.data?.data?.paymentPeriod,
        beneficiaryName: resp?.data?.data?.beneficiaryName,
      });
      ...
      <div className={isSlider ? "bodymain" : "bodymain collapsebody"}>
        <div className="breadcrum-top mb-4">
          <ul className="d-flex align-items-center gap-2">
            ...
            <li>/</li>
            <li className="text-capitalize">Edit Account Information</li>
          </ul>
        </div>
        <div className="row">
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
                <h5 className="mb-md-0">Edit Account Information</h5>
                <Link
                  href={getLinkHref(detail?.roleId, "/page/account-information")}
                  className="btn_theme"
                >
