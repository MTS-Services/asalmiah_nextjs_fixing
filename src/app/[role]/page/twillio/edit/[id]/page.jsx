...
import { EDIT_TWILLIO, GET_DETAILS_TWILLIO } from "../../../../../../../services/APIServices";
...
const TwilioForm = () => {
  ...
  // Mutation for adding/editing Twilio configuration
  const detail = useDetails()
  const { mutate, isPending } = useMutation({
    mutationFn: (payload) => EDIT_TWILLIO(id, payload),
    onSuccess: (resp) => {
      toastAlert("success", resp?.data?.message);
      resetForm();
      router.push(getLinkHref(detail?.roleId, `/page/twillio`));
      queryClient.invalidateQueries({ queryKey: ["twilio-list"] });
    },
  });

  // Fetch Twilio details
  useQuery({
    queryKey: ["twilio-detail", id],
    queryFn: async () => {
      const resp = await GET_DETAILS_TWILLIO(id);
      setValues({
        twilioAccountSid: resp?.data?.data?.twilioAccountSid,
        twilioAuthToken: resp?.data?.data?.twilioAuthToken,
        twilioWhatsAppNumber: resp?.data?.data?.twilioWhatsAppNumber,
        twilioServiceId: resp?.data?.data?.twilioServiceId,
        twilioPhoneNumber: resp?.data?.data?.twilioPhoneNumber,
        twilioContentSid: resp?.data?.data?.twilioContentSid,
      });
      ...
      <div className="bodymain">
        <div className="breadcrum-top mb-4">
          <ul className="d-flex align-items-center gap-2">
            ...
            <li>/</li>
            <li className="text-capitalize">Edit Twilio</li>
          </ul>
        </div>
        <div className="row">
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
                <h5 className="mb-md-0">Edit Twilio</h5>
                <Link
                  href={getLinkHref(detail?.roleId, "/page/twillio")}
                  className="btn_theme"
                >
