...
const Edit = () => {
  ...
  const { mutate, error, isPending } = useMutation({
    mutationFn: (payload) => EDIT_BRANCH_API(id, payload),
    onSuccess: (resp) => {
  ...
  useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const resp = await GET_BRANCH_DETAIL(id);
      setValues({
        ...values,
        branchName: resp?.data?.data?.branchName,
        ...
        isDeliveryPoint: resp?.data?.data?.isDeliveryPoint,
        isCouponBranch: resp?.data?.data?.isCouponBranch,
        // deliveryWhatsUpNo: resp?.data?.data?.deliveryWhatsUpNo,
        // costDelivery: resp?.data?.data?.costDelivery,
        deliveryEmail: resp?.data?.data?.deliveryEmail,
        companyId: {
          label: resp?.data?.data?.companyId?.company,
          value: resp?.data?.data?.companyId?._id,
        },
        workingHours: resp?.data?.data?.workingHours,
        deliveryWhatsUpNo:
          resp?.data?.data?.CountryCode + resp?.data?.data?.deliveryWhatsUpNo,
        //  ARABIC
        arabicBranchName: resp?.data?.data?.arabicBranchName,
      });
      ...
      <div className={isSlider ? "bodymain" : "bodymain collapsebody"}>
        <div className="breadcrum-top mb-4">
          <ul className="d-flex align-items-center gap-2">
            ...
            <li>/</li>
            <li className="text-capitalize">Edit Branch</li>
          </ul>
        </div>
        <div className="row">
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
                <h5 className="mb-md-0">Edit Branch</h5>
                <Link
                ...
                <form onSubmit={handleSubmit}>
                  <Row>
                    ...
                    {/* <Col md={6} className="mx-auto">
                      <Form.Group className="mb-4">
                        <Form.Label>
                          Delivery Cost<span className="text-danger">*</span>
                        </Form.Label>
                        <Form.Control
                          className="form-control"
                          type="text"
                    ...
                    <Col lg={6}>
                      <div className="mb-4">
                        ...
                        <PhoneInput
                          defaultCountry="kw"
                          placeholder="Enter Delivery WhatsUpNo"
                    ...
                    {/* <Col lg={12}>
                      <div className="active-radio d-flex algn-items-center  mb-2">
                        <Form.Group className="mt-3">
                          <Form.Label>Delivery Whatts App Number</Form.Label>
                          <Form.Check
                            name="deliveryWhatsUpNo"
                            type="radio"
                            label="true"
                            id="true"
                            checked={values?.deliveryWhatsUpNo == 1}
                            value={1}
                            onChange={handleChange}
                          />
                          <Form.Check
                            ...
                            label="false"
                            id="false"
                            checked={values?.deliveryWhatsUpNo == 0}
                            value={0}
                            onChange={handleChange}
                          />
                        </Form.Group>
