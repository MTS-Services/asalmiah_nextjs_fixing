...
import {
  DELETE_DELIVERY_COMPANY_API,
  GET_DELIVERY_COMPANY_API,
  STATE_DELIVERY_COMPANY_API,
...
const DeliveryCompanyManagement = () => {
  ...
  const getData = async (page, search, state = "") => {
    try {
      const response = await GET_DELIVERY_COMPANY_API(page, search, state);
  ...
  const { mutate } = useMutation({
    mutationFn: (payload) => DELETE_DELIVERY_COMPANY_API(payload),
    onSuccess: (resp) => {
  ...
  const stateMutation = useMutation({
    mutationFn: (body) => STATE_DELIVERY_COMPANY_API(body?.id, body?.state),
    onSuccess: (resp) => {
      toastAlert("success", resp?.data?.message);
      // refetch();
      getData(page, search, state);
  ...
  const deleveryManagementPermissions = getPermissionsByLabel(
    ...
    <div className={isSlider ? "bodymain fullbody" : "bodymain "}>
      <div className="breadcrum-top mb-4">
        <ul className="d-flex align-items-center gap-2">
          ...
          <li className="text-capitalize">Delivery Company management</li>
        </ul>
      </div>
      <div className="row">
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
              <h4 className="mb-md-0">Delivery Company</h4>

              <div className="filter_dropdown flex-wrap">
                {(deleveryManagementPermissions?.at(0)?.value?.subNav?.deliveryCompany.add == true &&
                  (detail?.roleId === constant.DESIGNED_USER ||
                    detail?.roleId === constant.PROMOTION_USER)) ||
                  detail?.roleId == constant.ADMIN ? (
                  <Link
                    href={getLinkHref(detail?.roleId, `/page/delivery-company-management/add`)}
                    className="btn_theme"
                  >
                          ...
                          <tr key={data?._id}>
                            ...
                            <td>
                              <div className="d-flex">
                                {(deleveryManagementPermissions?.at(0)?.value?.subNav
                                  ?.deliveryCompany.view ==
                                  true &&
                                  (detail?.roleId === constant.DESIGNED_USER ||
                                    detail?.roleId ===
                                  ...
                                  <Link
                                    ...
                                    href={getLinkHref(detail?.roleId, `/page/delivery-company-management/view/${data?._id}`)}
                                  >
                                    {" "}
                                    <Button
                                ...
                                {(deleveryManagementPermissions?.at(0)?.value?.subNav
                                  ...
                                  <Link
                                    ...
                                    href={getLinkHref(detail?.roleId, `/page/delivery-company-management/edit/${data?._id}`)}
                                  >
                                    {" "}
                                    <Button
                                      title="Edit"
