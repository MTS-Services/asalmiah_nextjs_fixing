...
import {
  DELETE_DYNAMICLABEL_API,
  GET_DYNAMICLABEL_API,
  STATE_UPDATE_DYNAMICLABEL_API
...
const DynamicLabel = () => {
  ...
  const getData = async (page, search, state = "") => {
    try {
      const response = await GET_DYNAMICLABEL_API(page, search, state);
  ...
  const { mutate } = useMutation({
    mutationFn: (payload) => DELETE_DYNAMICLABEL_API(payload),
    onSuccess: (resp) => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You want to delete this dynamic labelling!",
      icon: "warning",
  ...
  const stateMutation = useMutation({
    mutationFn: (body) => STATE_UPDATE_DYNAMICLABEL_API(body?.id, body?.state),
    onSuccess: (resp) => {
      toastAlert("success", resp?.data?.message);
      // refetch();
      getData(page, search, state); // Ensure we fetch the updated data
    ...
    <div className={isSlider ? "bodymain fullbody" : "bodymain "}>
      <div className="breadcrum-top mb -4">
        <ul className="d-flex align-items-center gap-2">
          ...
          <li>/</li>
          <li className="text-capitalize">dynamic label</li>
        </ul>
      </div>
                          ...
                          <tr key={data?._id}>
                            ...
                            <td>
                              <div className="d-flex">
                                {(settingsPermissions?.at(0)?.value
                                  ?.subNav?.dynamicLabel?.view == true &&
                                  (detail?.roleId === constant.DESIGNED_USER ||
                                    detail?.roleId ===
                                    constant.PROMOTION_USER)) ||
                                  detail?.roleId == constant.ADMIN ? (
                                  <Link
                                    href={getLinkHref(detail?.roleId, `/page/dynamic-label/view/${data?._id}`)}
                                ...
                                {(settingsPermissions?.at(0)?.value
                                  ?.subNav?.dynamicLabel?.edit == true &&
                                  (detail?.roleId === constant.DESIGNED_USER ||
                                    detail?.roleId ===
                                    constant.PROMOTION_USER)) ||
                                  detail?.roleId == constant.ADMIN ? (
                                  <Link
                                    href={getLinkHref(detail?.roleId, `/page/dynamic-label/edit/${data?._id}`)}
                                ...
                                {(settingsPermissions?.at(0)?.value
                                  ?.subNav?.dynamicLabel?.delete == true &&
                                  (detail?.roleId === constant.DESIGNED_USER ||
                                    detail?.roleId ===
                                    constant.PROMOTION_USER)) ||
                                  detail?.roleId == constant.ADMIN ? (
