...
const AccountInformation = () => {
  ...
  const getData = async (page, search, state = "") => {
    try {
      const response = await GET_ACCOUNT_LIST(page, search, state);
  ...
  const { mutate } = useMutation({
    mutationFn: (payload) => DELETE_ACCOUNT_ID(payload),
    onSuccess: (resp) => {
  ...
  const stateMutation = useMutation({
    mutationFn: (body) => STATE_UPDATE_ACCOUNT(body?.id, body?.state),
    onSuccess: (resp) => {
      toastAlert("success", resp?.data?.message);
      getData(page, search, state);
    },
  ...
  const accountManagementPermissions = getPermissionsByLabel(
    ...
    <div className={isSlider ? "bodymain fullbody" : "bodymain "}>
      ...
      <div className="breadcrum-top mb-4">
        <ul className="d-flex align-items-center gap-2">
          ...
          <li className="text-capitalize">Account Information</li>
        </ul>
      </div>
      <div className="row">
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header d-flex justify-content-between align-items-center flex-wrap">
              <h4 className="mb-md-0">Account Information</h4>

              <div className="filter_dropdown flex-wrap">


                {(accountManagementPermissions?.at(0)?.value?.subNav?.accountInformation?.add == true && (detail?.roleId === constant.DESIGNED_USER || detail?.roleId === constant.PROMOTION_USER)) ||
                  detail?.roleId == constant.ADMIN ? (
                  <Link
                    href={getLinkHref(detail?.roleId, `/page/account-information/add`)}
                    className="btn_theme"
                  >
                ...
                <table className="table table-striped">
                  <thead>
                    <tr>
                      ...
                      <th>Accountant Telephone</th>
                      <th>Payment Method</th>
                      <th>Payment Period/Day</th>
                      <th>Flexible Price</th>
                      <th>Cheque Company name</th>
                      <th>Link Telephone Number</th>
                      ...
                      sortedData?.map((data, index) => {
                        return (
                          <tr key={data?._id}>
                            ...
                            <td>{accountTypeFunc(data?.accountType ?? "-")}</td>
                            <td>{data?.accountantName ?? "-"}</td>
                            <td>{data?.accountantTelephone ?? "-"}</td>
                            <td>
                              {paymentTypeFunc(data?.paymentMethod ?? "-")}
                            </td>
                            <td>{data?.paymentPeriod ?? "-"}</td>
                            <td>
                              {data?.flexiblePrice == true
                                ? "Yes"
                                : "No" ?? "-"}
                            </td>
                            <td>{data?.chequeCompany ?? "-"}</td>
                            <td>{data?.linkTelephoneNumber ?? "-"}</td>
                            ...
                            <td>
                              <div className="d-flex">
                                {(accountManagementPermissions?.at(0)?.value?.subNav?.accountInformation?.view == true && (detail?.roleId === constant.DESIGNED_USER || detail?.roleId === constant.PROMOTION_USER)) ||
                                  detail?.roleId == constant.ADMIN ? (
                                  <Link

                                    href={getLinkHref(detail?.roleId, `/page/account-information/view/${data?._id}`)}
                                  >
                                    {" "}
                                    <Button
                                ...
                                {(accountManagementPermissions?.at(0)?.value?.subNav?.accountInformation?.edit == true && (detail?.roleId === constant.DESIGNED_USER || detail?.roleId === constant.PROMOTION_USER)) ||
                                  detail?.roleId == constant.ADMIN ? (
                                  <Link

                                    href={getLinkHref(detail?.roleId, `/page/account-information/edit/${data?._id}`)}
                                ...
                                {(accountManagementPermissions?.at(0)?.value?.subNav?.accountInformation?.delete == true && (detail?.roleId === constant.DESIGNED_USER || detail?.roleId === constant.PROMOTION_USER)) ||
                                  detail?.roleId == constant.ADMIN ? (
                                  <Button
                                    title="Delete"
